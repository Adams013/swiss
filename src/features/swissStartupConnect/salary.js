...
export const THIRTEENTH_SALARY_PATTERN = /\b(?:13(?:th)?|thirteenth)\b/i;
...
export const getStepFactor = (step) => {
  if (!Number.isFinite(step) || step <= 0) {
    return 1;
  }
  const decimals = step.toString().split('.')[1];
  return decimals ? 10 ** decimals.length : 1;
};

export const alignToStep = (value, step, strategy) => {
  ...
  const scaledStep = step * factor;
  return strategy(scaledValue / scaledStep) * step;
};
...
export const formatEquityDisplay = (min, max) => {
  ...
  const formattedMin = hasMin ? formatEquityValue(min) : '';
  const formattedMax = hasMax ? formatEquityValue(max) : '';
...
export const detectSalaryPeriod = (job, salaryText) => {
  ...
  for (const field of SALARY_PERIOD_FIELDS) {
  ...
  const text = typeof salaryText === 'string' ? salaryText.toLowerCase() : '';
  if (text.includes('per hour') || text.includes('/ hour')) {
    return 'hour';
  }
  if (text.includes('per week') || text.includes('/ week')) {
    return 'week';
  }
  if (text.includes('per year') || text.includes('/ year')) {
    return 'year';
  }
  if (text.includes('per day') || text.includes('/ day')) {
    return 'day';
  }
  if (text.includes('per month') || text.includes('/ month')) {
    return 'month';
  }
...
export const normalizeSalaryCadence = (value) => {
  ...
  const normalized = value.trim().toLowerCase();
  if (['hourly', 'hour', 'per_hour'].includes(normalized)) {
    return 'hour';
  }
  if (['weekly', 'week', 'per_week'].includes(normalized)) {
    return 'week';
  }
  if (['annual', 'yearly', 'year', 'per_year'].includes(normalized)) {
    return 'year';
  }
  return 'month';
};
