 ...
 * 
 * Setup:
 * 1. Configure REACT_APP_SUPABASE_URL/REACT_APP_SUPABASE_ANON_KEY or REACT_APP_AI_PROXY_URL
 * 2. Keep provider API keys on the server (Edge Function, serverless proxy, etc.)
 */
...
const AI_PROXY_URL = process.env.REACT_APP_AI_PROXY_URL || process.env.REACT_APP_AI_CHAT_URL;
...
/**
 * Chat with AI using a secure backend
 * Requires either a Supabase Edge Function or custom proxy endpoint
 */
export const chatWithAI = async (messages, options = {}) => {
  try {
    if (AI_PROXY_URL) {
      return await chatViaProxy(messages, options);
    }
    ...
    throw new Error('AI chat backend not configured');
  } catch (error) {
...
/**
 * Chat via custom proxy endpoint
 */
const chatViaProxy = async (messages, options = {}) => {
  try {
    ...
    console.error('AI Chat via proxy error:', error);
    return {
      message: null,
      error: error.message,
    };
